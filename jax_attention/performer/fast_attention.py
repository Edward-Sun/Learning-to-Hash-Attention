...
def old_nonnegative_softmax_kernel_feature_creator(data,
                                                   projection_matrix,
                                                   attention_dims_t,
                                                   batch_dims_t,
                                                   precision,
  ...
  if is_query:
    ...
    data_dash = ratio * (
        jnp.exp(data_dash - diag_data - jnp.max(data_dash)) +
        eps)
...
def make_fast_softmax_attention(qkv_dim,
  ...
  attention_fn = FastAttentionviaLowRankDecomposition(
      ...
      key_padding_mask=key_padding_mask).dot_product_attention
  return attention_fn
...
class GaussianOrthogonalRandomMatrix(RandomMatrix):
  ...
  def get_2d_array(self):
    ...
    for _ in range(nb_full_blocks):
      ...
      q, r = jnp.linalg.qr(unstructured_block)
      di = jnp.diag(r, 0)
      ph = jnp.sign(di)
      q *= ph
      q = jnp.transpose(q)
...
class FastAttentionviaLowRankDecomposition(FastAttention):
  ...
  def __init__(self,
               ...
               unidirectional,
               lax_scan_unroll=1,
               key_padding_mask=False):  # For optimal GPU performance, set to 16.
  ...
  def dot_product_attention(self,
    ...
    if self.key_padding_mask:
      logging.info(bias)
      # k_mask -> (bs, <non-attention dims>, num_heads, <attention dims>, 1)
      padding_mask_bias = jnp.mean(bias, axis=-2)
      padding_mask_bias = jnp.expand_dims(padding_mask_bias, axis=-1)
      logging.info(padding_mask_bias)
      padding_mask = lax.select(
          # See https://github.com/google/flax/blob/3e36db3e5e3b8e6e1777d612f270e7948238aa9c/flax/nn/attention.py#L396
          # for this magic number
          padding_mask_bias > -1e10 + 1.0,
          jnp.full(padding_mask_bias.shape, 1.0).astype(dtype),
          jnp.full(padding_mask_bias.shape, 0.0).astype(dtype))
      key_prime = key_prime * padding_mask
